...
const Pri2 = () => {
  ...
  useEffect(() => {
    const activePri2 = saved.filter(
      (task) => task.status !== false && task.priority === "priority-2"
    );
    setTaskcount(activePri2.length);
  }, [saved]);
  ...
  const renderTasks = () =>
    saved
      .filter(
        (task) =>
          task.status !== false && task.priority === "priority-2"
      )
  ...
  return (
    <div className={styles.mainArea}>
      <h1 className={styles.Today}>Priority 2 Tasks</h1>
